...
uint32_t task_send_periodic_msg(void)
{
    ...
    payload.sensor0 = pData->temp;
    payload.sensor1 = pData->humidity;
    payload.sensor2 = pData->pressure;
    payload.sensor3 = pData->battery;
    payload.sensor4 = pData->rssi;
    ...
    payload.sensor0, payload.sensor1, payload.sensor2, 
    payload.sensor3, payload.sensor4, payload.count);
...
void send_uttec_response_msg(payload_demo_t* pUttec)
{
    // payload_uttec_t payload; 
    /* Send message. */
        ...
        const app_lib_data_received_t * data)
{
    ...
    Printf("msgId: %d\r\n", receive_msg.id);
    Printf("area: %d\r\n", receive_msg.payload.demo.area_id);
    Printf("rack: %d\r\n", receive_msg.payload.demo.rack_id);
    Printf("ctr: %d\r\n", receive_msg.payload.demo.ctr);
    Printf("relay: %d\r\n", receive_msg.payload.demo.relay);
    int16_t iCtr = receive_msg.payload.demo.ctr;
    int16_t iRelay = receive_msg.payload.demo.relay;
    // if(iRelay == 3){
    //     beep(20);
    // }
    // else{
    //     Led_set(iRelay - 1, iCtr);
    // }
    writeOutSx(iRelay, iCtr);
    ...
    data_t* pSensor = getMySensor();
    pSensor->rssi = rssi;
    // send_uplink_msg('A', );
    payload_demo_t tempMsg;
    tempMsg.area_id = getAreaId();
    tempMsg.rack_id = getMyId();
    tempMsg.ctr = 0;
    tempMsg.relay = read_temp();
    tempMsg.sensor_0 = 25;
    tempMsg.sensor_1 = 26;
    tempMsg.rssi = rssi;
    tempMsg.battery = rssi;
    // setRssi(rssi);
