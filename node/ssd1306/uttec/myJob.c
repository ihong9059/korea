...
void doMyJob(void){
    ...
    if(!(countMyJob%10)){
        ...
        if(toggle){
            sprintf(line,"%02d %02d:%02d %01d", 
            pTime->day, pTime->hour, pTime->minute, (myJobSec % 10));
        }
        else{
            sprintf(line,"C:%02d A:%02d ", 
            pFlash->company, pFlash->areaId);
        }
        if(pDevice->lcdFlag){
            dispLcdLine(line, 0);
        }
        ...
        pData->battery = battery / 10;

        int16_t red = getAdc(2)*2;
        if(red < 0) red = 0;
        pData->red = red;

        // pData->battery = getAdc(0)*2;
        if(pData->battery < 390){
            if(chargeFlag){
                Led_set(BatCtr, false); //high, charge
                Printf("Charging");
            }
            else{
                Led_set(BatCtr, true); //low, cut solar cell power
                Printf("dis-----------Charging\r\n");
            }
        }
        else{
